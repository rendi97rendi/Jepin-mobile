...
import 'CulinaryDetail.dart';

String? _urlImage;

class CulinaryList extends StatefulWidget {
  final String title;
  const CulinaryList({Key? key, required this.title}) : super(key: key);

  @override
  _CulinaryListState createState() => _CulinaryListState();
}

class _CulinaryListState extends State<CulinaryList> {
  // --- variable ---
  String _category = "";
  List _listRestaurantCategory = [];
  var _loadingRestaurantCategory = true;
  ...
  Widget build(BuildContext context) {
    ...
    final _restaurantList = _dataLoading
        ? LayoutLoading()
        : Expanded(
            child: SmartRefresher(
              ...
              header: defaultTargetPlatform == TargetPlatform.iOS
                  ? WaterDropHeader()
                  : WaterDropMaterialHeader(backgroundColor: Colors.orange),
              ...
              child: ListView(
                ...
                children: <Widget>[
                  GridView.builder(
                      ...
                      itemCount:
                          _listRestaurant == null ? 0 : _listRestaurant.length,
                            ...
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                  builder: (context) => CulinaryDetail(
                                        id: _listRestaurant[index]["id"]
                                            .toString(),
                                      )),
                            );
                          },
                          child: _ViewHolderRestaurant(
                            image: _listRestaurant[index]["details"],
                            name: _listRestaurant[index]["nama"],
                            address: _listRestaurant[index]["alamat"],
                          ),
                        );
                      }),
                ],
  ...
  Future<String> showData(
    ...
    var response = await http.get(
      Uri.parse(ApiService.restaurantList + param),
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      if (result["status"] == "success") {
        ...
        if (clearListParent) _listRestaurant.clear();
  ...
  Future<String> restaurantCategory() async {
    setState(() {
      _loadingRestaurantCategory = true;
    });

    var response = await http.get(
      Uri.parse(ApiService.restaurantCategory),
      headers: {"Accept": "application/json"},
    );
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      if (result["status"] == "success") {
        _listRestaurantCategory = result["data"];
      } else {
        MyHelper.toast(context, MyString.msgError);
        ;
      }
    ...
    setState(() {
      _loadingRestaurantCategory = false;
    });
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: ListView.builder(
                        itemCount: _listRestaurantCategory == null
                            ? 1
                            : _listRestaurantCategory.length + 1,
                        itemBuilder: (BuildContext context, int index) {
                          if ((_listRestaurantCategory.length + 1) ==
                              (index + 1)) {
                            return _listRestaurantCategory == null
                                ? Container()
                                : Column(
                                    children: <Widget>[
                                      InkWell(
...
class _ViewHolderRestaurant extends StatelessWidget {
  final List image;
  final String name, address;

  const _ViewHolderRestaurant(
